...
打开时用**当下**数据在离屏 canvas 画 1080×1920：冰蓝渐变、品牌胶囊、主标题（本周好友数）、伴听陪跑榜 Top 3、三项统计（伴听分钟/合跑圈数/击掌）、情绪文案、日期和标语。生产模式顶部伴听时间以 `tracks.created_at` 为起点；每位好友以跑道创建时间和 `track_friends.joined_at` 中较晚者为起点，页面每秒刷新并在重新打开时从数据库时间恢复。Demo 模式继续使用预设模拟分钟数。Canvas 通过 `toDataURL` 生成 PNG → 预览 `<img>` + `<a download>` 下载，失败走 Toast，不阻塞。
