...
LANE_RX = [0.52, 0.63, 0.74]（盘面中孔~外缘之间的三条车道）；ry = rx × 0.94
...
### 5.4 唱片视觉分层（z-index 由内到外）

1. `.coverSpin`（z1）：专辑封面圆图，居中于盘面透明中孔下方，**恒定平面自转 26s/圈**（播放时转、暂停停、reduced-motion 关）。居中用负 margin 而非 transform，避免与旋转动画打架。
2. `.disk`（z2）：`music_player_disk.png` 盘面，**完全静止**。
