...
打开时用**当下**数据在离屏 canvas 画 1080×1920：冰蓝渐变、品牌胶囊、主标题（本周好友数）、伴听陪跑榜 Top 3、三项统计（伴听分钟/合跑圈数/击掌）、情绪文案、日期和标语。生产模式尚无真实伴听累计时，会根据真实好友稳定生成演示统计；同一好友不会在每次打开时随机跳变。Canvas 通过 `toDataURL` 生成 PNG → 预览 `<img>` + `<a download>` 下载，失败走 Toast，不阻塞。
